...
def evaluate(max_eval, render, fps, use_explorer, plot_data):
    ...
    for i, worker in enumerate(workers):
        champ, max_score = worker.results[-1][-1][0]
...
def evaluate_t_maze_task(worker: GA, nn: TMazeNTMModule, max_eval, render, fps):
    ...
    env = TMaze(length, 10, cyclic_order=False, view_size=view_size, double=double, uneven_rounds=True, goal_positions=[1, 0, 1, 0])
    for _ in range(100):
        reward, n_eval = nn.evaluate(env, max_eval, render=False)
        rewards.append(reward)
    nn.start_history()
